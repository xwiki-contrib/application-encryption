...
public class DefaultEncryptionTool implements EncryptionTool
{
    ...
    public boolean hasDecryptionRight()
    {
        try{
            ...
            logger.warn("Proper wiki : " + documentAccessBridge.getCurrentDocumentReference().getWikiReference().toString());
            ...
            if(nb >= 0)
            {
                ...
                Scanner scan = new Scanner(authorizedUsers.toString()).useDelimiter(",");
                while(scan.hasNext())
                {
            ...
            String space = this.documentAccessBridge.getCurrentDocumentReference().getSpaceReferences().get(0).getName();
            nb = this.documentAccessBridge.getObjectNumber(docRef, classRef, "name", space);
            if(nb >= 0)
            {
                Object authorizedUsersSpace = this.documentAccessBridge.getProperty(docRef, classRef, nb, "authorizedUsers");
                Scanner scanSpace = new Scanner(authorizedUsersSpace.toString()).useDelimiter(",");
                while(scanSpace.hasNext())
                {
